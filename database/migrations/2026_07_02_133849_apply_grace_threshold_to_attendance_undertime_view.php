    ...
    public function down(): void
    {
        DB::statement("
            ...
            SELECT
                ...
                r.post4_time        AS actual_timeout_PM,
                g.grace_minutes,
                GREATEST( COALESCE(TIMESTAMPDIFF(MINUTE, r.timein_AM,  r.post1_time), 0),                    0)  AS late_minutes_AM,
                GREATEST( COALESCE(TIMESTAMPDIFF(MINUTE, r.post2_time, r.timeout_AM), 0),                    0)  AS undertime_lunch_out_minutes,
                GREATEST( COALESCE(TIMESTAMPDIFF(MINUTE, r.timein_PM,  r.post3_time), 0),                    0)  AS late_minutes_PM,
                GREATEST( COALESCE(TIMESTAMPDIFF(MINUTE, r.post4_time, r.timeout_PM), 0),                    0)  AS undertime_end_of_day_minutes,
                CASE WHEN r.no_lunch_gap = 1 THEN (
                         GREATEST( COALESCE(TIMESTAMPDIFF(MINUTE, r.timein_AM,  r.post1_time), 0),                    0)
                     ...
                     ELSE (
                       ...
                       + GREATEST( COALESCE(TIMESTAMPDIFF(MINUTE, r.timein_PM,  r.post3_time), 0),                    0)
                       + GREATEST( COALESCE(TIMESTAMPDIFF(MINUTE, r.post4_time, r.timeout_PM), 0),                    0)
                     )
                END AS total_undertime_minutes
            FROM resolved r
